...
<div class="row">
    ...
    <div class="col-sm-5 card">
        <h4 class="display-10">Cập nhật Category</h4>
            <form class="p-3" method="POST">
            @include('backend.category.form')
                <button type="submit" class="btn btn-primary"  formaction="{{ route('get_backend.category.update', $category->id) }}" >Cập nhật</button>
                <button type="submit" class="btn btn-primary"  formaction="{{ route('get_backend.category.store') }}" >Thêm</button>
            </form>
    </div>
</div>
@stop
